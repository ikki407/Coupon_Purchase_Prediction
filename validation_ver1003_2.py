...
def top20_cos_sim_weight(data):
   top20_cos_sim_cp = data[data[data.argsort()[::-1].values][:30].index.values]
   count_weight_top = count_weight2.iloc[(count_weight2['coupon_id_hash'].isin(top20_cos_sim_cp.index.values)).values,:]
   return top20_cos_sim_cp.mul(count_weight_top['count_weight'].astype(float))

count_weight2['coupon_id_hash'] = count_weight2['coupon_id_hash'].astype(str)
cos_sim_weight7 = cos_sim.apply(top20_cos_sim_weight,axis=1).fillna(0)
...
cos_sim_weight = cos_sim_weight6
...
#ヘビーユーザー
heavy_user = cpdtr['user_id_hash'].value_counts()[cpdtr['user_id_hash'].value_counts()>=5].index.values
heavy_user_index = pred_ikki['USER_ID_hash'].isin(heavy_user)[pred_ikki['USER_ID_hash'].isin(heavy_user)==True].index

actual_and_pred.columns = ['actual','pred']
val_score = np.sum(actual_and_pred.ix[heavy_user_index].apply(run_apk,axis=1))

print 'AP@5のvalidationスコア: ', val_score
print 'MAP@5のvalidationスコア: ', val_score/len(ulist)

#ライトユーザー
light_user = cpdtr['user_id_hash'].value_counts()[cpdtr['user_id_hash'].value_counts()<5].index.values
light_user_index = pred_ikki['USER_ID_hash'].isin(light_user)[pred_ikki['USER_ID_hash'].isin(light_user)==True].index



actual_and_pred.columns = ['actual','pred']
val_score = np.sum(actual_and_pred.ix[light_user_index].apply(run_apk,axis=1))
