...
#欠損値補間
ulist_train['ITEM_COUNT'] = ulist_train['ITEM_COUNT'].fillna(1)
ulist_train['PURCHASE_FLG'] = ulist_train['PURCHASE_FLG'].fillna(1)
ulist_train['CATALOG_PRICE'] = ulist_train['CATALOG_PRICE'].fillna(1)
ulist_train['DISCOUNT_PRICE'] = ulist_train['DISCOUNT_PRICE'].fillna(1)
ulist_train['PRICE_RATE'] = ulist_train['PRICE_RATE'].fillna(1)
ulist_train['DISPPERIOD'] = ulist_train['DISPPERIOD'].fillna(1)
ulist_train['VALIDPERIOD'] = ulist_train['VALIDPERIOD'].fillna(1)
...
object_col = ulist_test.iloc[:,(ulist_test.dtypes==object).values].columns.drop(['USER_ID_hash','COUPON_ID_hash']+time_label)
...
del all_train_tr['I_DATE'], all_train_val['I_DATE'], all_test['I_DATE'],all_train['I_DATE']
...
all_train[all_train.columns.drop(['COUPON_ID_hash','PURCHASE_FLG'])].groupby('USER_ID_hash').mean().astype(float)
...
pred = pred.groupby('USER_ID_hash').apply(top10)
pred = pd.DataFrame(pred.values,columns=[u'USER_ID_hash',u'PURCHASED_COUPONS'])
pred.to_csv('/Users/IkkiTanaka/Documents/kaggle/Coupon_Purchase_Prediction/prediction/fastFM1.csv',index=False)
...
all_train[['AG10','AG20','AG30','AG40','AG50','AG60','AG70+']] = all_train['AGE'].apply(age_dummy).fillna(0)
all_test[['AG10','AG20','AG30','AG40','AG50','AG60','AG70+']] = all_test['AGE'].apply(age_dummy).fillna(0)
...
all_train[['CP1','CP100','CP1000','CP10000','CP100000']] = all_train['CATALOG_PRICE'].apply(catalog_price_dummy).fillna(0)
del all_train['CP1']
all_test[['CP100','CP1000','CP10000','CP100000']] = all_test['CATALOG_PRICE'].apply(catalog_price_dummy).fillna(0)
...
all_train[['DP0','DP1','DP10','DP100','DP1000','DP10000','DP100000']] = all_train['DISCOUNT_PRICE'].apply(discount_price_dummy).fillna(0)
del all_train['DP0'], all_train['DP1'], all_train['DP10'], all_train['DP100000']
all_test[['DP100','DP1000','DP10000']] = all_test['DISCOUNT_PRICE'].apply(discount_price_dummy).fillna(0)
...
all_train[['DI0-5','DI6-10','DI11-15','DI16-20','DI20+']] = all_train['DISPPERIOD'].apply(dispperiod_dummy).fillna(0)

all_test[['DI0-5','DI6-10','DI11-15','DI16-20','DI20+']] = all_test['DISPPERIOD'].apply(dispperiod_dummy).fillna(0)
...
all_train[['PR50-','PR50-60','PR60-70','PR70-80','PR80-90','PR90+']] = all_train['PRICE_RATE'].apply(price_rate_dummy).fillna(0)
all_test[['DP0','DP1','DP10','DP100','DP1000','DP10000','DP100000']] = all_test['PRICE_RATE'].apply(price_rate_dummy).fillna(0)


del all_train['AGE'],all_train['CATALOG_PRICE'],all_train['DISCOUNT_PRICE'],all_train['DISPPERIOD'],all_train['PRICE_RATE']
del all_test['AGE'],all_test['CATALOG_PRICE'],all_test['DISCOUNT_PRICE'],all_test['DISPPERIOD'],all_test['PRICE_RATE']
...
a = all_train[all_train.columns.drop(['COUPON_ID_hash','PURCHASE_FLG'])].fillna(0).groupby('USER_ID_hash').mean().astype(float)
user_list_test = pd.DataFrame(a.index,columns=['USER_ID_hash'])
a = np.matrix(a)

b = all_test[all_test.columns.drop(['USER_ID_hash'])].fillna(0).groupby('COUPON_ID_hash').mean().astype(float)
...
submission = ulist.merge(user_list_test,on=u'USER_ID_hash',how='left')
