...
train = train[[u'item_count', u'coupon_id_hash', u'genre_name',
...
test = cplte[[u'coupon_id_hash', u'genre_name',
       u'price_rate', u'catalog_price', u'discount_price', u'dispperiod', u'validperiod',
       u'usable_date_before_holiday', u'large_area_name', u'ken_name',
       u'small_area_name']]
...
#train_and_test.loc[(train_and_test['genre_name'].isin(['宅配','その他のクーポン'])).values,['large_area_name','ken_name','small_area_name']] = '本社'
...
test = train_and_test.iloc[len(train):,:]
...
#直近4日間に購入したジャンルのクーポンは推薦しない(宅配・グルメ以外)
#user_id,chokkin
def chokkin_cp_list(data):
    genre = data['genre_name'].values
    genre = filter(lambda x: x != 'グルメ', genre)
    genre = filter(lambda x: x != '宅配', genre)
    return genre

chokkin_cpdtr = cpdtr.iloc[(cpdtr['i_date']>='2012-06-20').values,:]
chokkin_cpdtr = chokkin_cpdtr.merge(cpltr,on='coupon_id_hash',how='inner')

chokkin_cp = chokkin_cpdtr.groupby('user_id_hash').apply(chokkin_cp_list)
chokkin_cp = pd.DataFrame(chokkin_cp,columns=['chokkin_cp'])
chokkin_cp['user_id_hash'] = chokkin_cp.index
chokkin_cp.index = range(0,len(chokkin_cp))

def chokkin_test_cp(data):
    chokkin_test = cplte.iloc[(cplte['genre_name'].isin(data)).values,:]['coupon_id_hash'].values.tolist()
    #print chokkin_test
    if len(chokkin_test)==0:
        return np.nan
    else:
        return chokkin_test

#直近に購入したクーポンと同じジャンルのクーポンをchokkin_test_cpに格納。ここに入っているクーポンは推薦しない
chokkin_cp['chokkin_test_cp'] = chokkin_cp['chokkin_cp'].apply(chokkin_test_cp)
chokkin_cp = chokkin_cp.dropna()
...
def syuhen_rec(data):
	...
	if (kyojuti =='') or (type(kyojuti) == float):#居住地なし
		...
		cplist_syuhen = cpate.iloc[(cpate['pref_name'].isin(convert_to_syuhen(kyojuti))).values,:].drop_duplicates('coupon_id_hash')['coupon_id_hash'].values.tolist()
		V2.values[~(V2.index.isin(cplist_syuhen)),:]=0
		#直近に買ったジャンルのcpは推薦しない
		
		if user_id_ in chokkin_cp['user_id_hash'].values:
                    chokkin_test_cp2 = chokkin_cp.iloc[(chokkin_cp['user_id_hash']==user_id_).values,:]['chokkin_test_cp'].values.tolist()[0]
		    #chokkin_test_cp2 = chokkin_test_cp2[0]
		    #print chokkin_test_cp2,type(chokkin_test_cp2)
                    #print V2.index.isin(chokkin_test_cp2)
		    V2.values[(V2.index.isin(chokkin_test_cp2)),:]=0
                    #print V2.values[(V2.index.isin(chokkin_test_cp2)),:]
                    print V2.values[(V2.index.isin(chokkin_test_cp2)),:].sum()
...
ans = pred_all.copy()
ans = ans[:(len(ulist)*5)]#top5までのレコメンドを見る
...
pred_ikki.to_csv('/Users/IkkiTanaka/Documents/kaggle/Coupon_Purchase_Prediction/prediction/ikki0911_pred32.csv',index=False)
