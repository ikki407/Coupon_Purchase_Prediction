...
#時間見ない
train = train[["COUPON_ID_hash","USER_ID_hash","GENRE_NAME","DISCOUNT_PRICE",'PRICE_RATE',"large_area_name","ken_name","small_area_name",u'I_DATE']]
...
cpchar = cplte[["COUPON_ID_hash","USER_ID_hash","GENRE_NAME","DISCOUNT_PRICE",'PRICE_RATE',"large_area_name","ken_name","small_area_name"]]
...
#max_time = train[u'I_DATE'].max()
#train[u'forget_factor'] = train[u'I_DATE'].apply(lambda x: (0.9999)**(max_time-x))
#a = train[u'forget_factor']
#train[train.columns.drop([u'COUPON_ID_hash',u'USER_ID_hash',u'I_DATE'])] = train[train.columns.drop([u'COUPON_ID_hash',u'USER_ID_hash',u'I_DATE'])].astype(float).rmul(train[u'forget_factor'],axis=0)

#del train[u'forget_factor']
...
#20120501より後
'''
train = train.iloc[(train[u'I_DATE']>=(datetime.datetime(2012,03,01).toordinal() - 734318)).values,:]
...
#train = train.iloc[(train['user_count'] > 1).values,:]#10回以上
...
#重み行列 DISCOUNT_PRICE PRICE_RATE USABLE_DATE_ GENRE_NAME ken_name large_area_name small_area_name
x1 = np.array([32,32,2,8,0.5,4])
x2 = np.repeat(x1, [1,1,13,47,9,55], axis=0)

W = np.matrix(np.diag(x2))
'''
#重み行列  DISCOUNT_PRICE u'I_DATE' PRICE_RATE USABLE_DATE_ GENRE_NAME large_area_name ken_name small_area_name DP_ 
x1 = np.array([2,3,0,0,3,3,4,0.5])
...
#ユーザとクーポンのコサイン類似度計算
score = np.matrix((uchar.iloc[:,:-1].T/((uchar.iloc[:,:-1]*uchar.iloc[:,:-1]).sum(1)**0.5)).T).dot(W).dot(np.matrix((test.iloc[:,1:].T/((test.iloc[:,1:]*test.iloc[:,1:]).sum(1)**0.5)).T).T)


#類似度に従ってクーポンをソートし、上位１０個を取る
#小山君の分析したcsv使用して各都道府県毎に決め打ちする

def top10(data,score):
    #print data['USER_ID_hash'].values[0]
    print ulist.iloc[(ulist['USER_ID_hash']==data['USER_ID_hash'].values[0]).values,:]['PREF_NAME'].values[0]
    ken = ulist.iloc[(ulist['USER_ID_hash']==data['USER_ID_hash'].values[0]).values,:]['PREF_NAME'].values[0]
    score2 = score
    #if type(ken) == str:#居住地があるならば
    if ken in ['東京都','沖縄県','北海道']:
        #print ken
        if ken in ['東京都']:
            a = cplte.iloc[(cplte['ken_name'].isin(['東京都','大阪府','神奈川県'])).values,:].index
        elif ken in ['北海道']:
            a = cplte.iloc[(cplte['ken_name'].isin(['東京都','大阪府','北海道'])).values,:].index
        elif ken in ['沖縄県']:
            a = cplte.iloc[(cplte['ken_name'].isin(['東京都','大阪府','沖縄県'])).values,:].index
        b = pd.Series(range(0,len(test)))
        b = b.drop(a).values#推薦しないクーポンのindex
        #print b,a
        #print score2[data.index[0]]
        score2[data.index[0]][0,[b]] = 0
        #print score2[data.index[0]]
    #print data.index[0]
    top10_cp = test['COUPON_ID_hash'].ix[score2[data.index[0]].argsort().tolist()[0][::-1][:10]]
    top10_cp = ' '.join(top10_cp)
    #print top10_cp
    return top10_cp
...
submission = submission.sort("USER_ID_hash")
submission.to_csv('/Users/IkkiTanaka/Documents/kaggle/Coupon_Purchase_Prediction/prediction/simple_pred_wcos1.csv',index=False)
