...
#直近1ヶ月に購入していないユーザーには宅配に重みをおいて推薦する
user_buy_1month = cpdtr.iloc[(cpdtr['i_date']>='2012-03-24').values,:]['user_id_hash'].drop_duplicates('user_id_hash')
#1ヶ月以内に購入していないユーザー
user_not_buy_1month = ulist.iloc[~(ulist['user_id_hash'].isin(user_buy_1month)).values]['user_id_hash'].values
#宅配クーポンの重み
takuhai_cp = cplte.iloc[(cplte['genre_name']=='宅配').values,:]['coupon_id_hash'].values
...
def convert_to_syuhen(data):
    if data in ['北海道']:
        ...
        return ['沖縄県','北海道','東京都','大阪府','神奈川県','千葉県']
    else:
        return []
...
o = pred_final
#'coupon_id_rank1','coupon_id_rank2','coupon_id_rank3','coupon_id_rank4','coupon_id_rank5']
user_notin_cpdtr = ulist.iloc[~(ulist['user_id_hash'].isin(cpdtr['user_id_hash'].drop_duplicates())).values,:]['user_id_hash'].values
...
pred_ikki.index = range(0,len(pred_ikki))
pred_ikki.columns = [['USER_ID_hash','PURCHASED_COUPONS']]
pred_ikki.to_csv('/Users/IkkiTanaka/Documents/kaggle/Coupon_Purchase_Prediction/prediction/ikki0911_pred24.csv',index=False)
