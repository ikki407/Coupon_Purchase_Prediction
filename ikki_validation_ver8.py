...
user_buy_1month = cpdtr.iloc[(cpdtr['i_date']>='2012-05-17').values,:]['user_id_hash'].drop_duplicates('user_id_hash')
#1ヶ月以内に購入していないユーザー
...
#特徴量を選択する(small_area_name_xは購入者の場所)
feature_label = [u'coupon_id_hash', u'user_id_hash',u'genre_name',u'large_area_name', u'ken_name',u'small_area_name_y','discount_price','price_rate','catalog_price','validperiod']
all_data = user_item_tr[feature_label]
all_data.columns = [u'coupon_id_hash', u'user_id_hash',u'genre_name',u'large_area_name', u'ken_name',u'small_area_name','discount_price','price_rate','catalog_price','validperiod']
...
#trainとtestを結合する
feature_label_te = [u'coupon_id_hash',u'genre_name',u'large_area_name', u'ken_name',u'small_area_name','discount_price','price_rate','catalog_price','validperiod']
#結合する前にTrainの長さを保存する
len_tr = len(all_data)
all_data = pd.concat([all_data,cplte[feature_label_te]])
all_data.index = range(0,len(all_data))
...
#validperiodのバイナリ化
vp_dic = {'vp0_15':pd.Series([1],index=['vp0_15']),'vp16_45':pd.Series([1],index=['vp16_45']),
'vp46_75':pd.Series([1],index=['vp46_75']),'vp76_105':pd.Series([1],index=['vp76_105']),'vp106_135':pd.Series([1],index=['vp106_135']),
'vp136_165':pd.Series([1],index=['vp136_165']),'vp166_195':pd.Series([1],index=['vp166_195']),'vp196+':pd.Series([1],index=['vp196+'])}
def validperiod_to_bi(data):
	#print data
	if data <= 15:
		#print data
		return vp_dic['vp0_15']
	elif 15 < data <= 45:
		return vp_dic['vp16_45']
	elif 45 < data <= 75:
		return vp_dic['vp46_75']
	elif 75 < data <= 105:
		return vp_dic['vp76_105']
	elif 105 < data <= 135:
		return vp_dic['vp106_135']
	elif 135 < data <= 165:
		return vp_dic['vp136_165']
	elif 165 < data <= 195:
		return vp_dic['vp166_195']
	elif 195 < data:
		return vp_dic['vp196+']

all_data['validperiod'] = all_data['validperiod'].fillna(999)
all_data['validperiod'] = all_data['validperiod'].astype(int)
a = all_data['validperiod'].apply(validperiod_to_bi).fillna(0)
all_data = pd.concat([all_data,a],axis=1)


#all_data[['pr50-','pr50-60','pr60-70','pr70-80','pr80-90','pr90+']] = all_data['price_rate'].apply(price_rate_to_bi).fillna(0)

del all_data['validperiod']
...
pred_ikki.index = range(0,len(pred_ikki))
pred_ikki.columns = [['USER_ID_hash','PURCHASED_COUPONS']]
pred_ikki.to_csv('/Users/IkkiTanaka/Documents/kaggle/Coupon_Purchase_Prediction/prediction/ikki0911_pred20.csv',index=False)
