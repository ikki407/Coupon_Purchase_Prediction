...
cplte = cplte.iloc[(cplte['DISCOUNT_PRICE']<10000).values,:]
...
#欠損値補間
train['DISCOUNT_PRICE'] = train['DISCOUNT_PRICE'].fillna(-999)
train['PRICE_RATE'] = train['PRICE_RATE'].fillna(-999)
...
train[['DP0','DP10','DP100','DP1000','DP10000','DP100000']] = train['DISCOUNT_PRICE'].apply(discount_price_dummy).fillna(0)
...
train[['PR50-','PR50-60','PR60-70','PR70-80','PR80-90','PR90+']] = train['PRICE_RATE'].apply(price_rate_dummy).fillna(0)

del train['DISCOUNT_PRICE'],train['PRICE_RATE']

#train['DISCOUNT_PRICE'] = 1/np.log10(train['DISCOUNT_PRICE'])
#train['PRICE_RATE'] =  1/np.log10(train['PRICE_RATE'])
#train['DISCOUNT_PRICE'] = train['DISCOUNT_PRICE']**(-.3)
#train['PRICE_RATE'] =  train['PRICE_RATE']**(-.3)
...
#20120501より後
'''
train = train.iloc[(train[u'I_DATE']>=(datetime.datetime(2012,04,23).toordinal() - 734318)).values,:]
...
x1 = np.array([1,0,0,2,3,1,4])
x2 = np.repeat(x1, [1,1,9,13,47,9,55], axis=0)
...
submission = submission.sort("USER_ID_hash")
submission.to_csv('/Users/IkkiTanaka/Documents/kaggle/Coupon_Purchase_Prediction/prediction/simple_pred12.csv',index=False)
...
submission = submission[["USER_ID_hash","PURCHASED_COUPONS"]]
submission["PURCHASED_COUPONS"] = submission["PURCHASED_COUPONS"].fillna('')
submission.to_csv('/Users/IkkiTanaka/Documents/kaggle/Coupon_Purchase_Prediction/prediction/simple_pred5.csv',index=False)
